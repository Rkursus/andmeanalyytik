...
Andmestiku saate sisse laadida järgmise käsu abil andes ette faili asukoha:
...
Suuruse _k_ saame ise määrata, näitek _k_ = 3 puhul oleks tulemus järgmine:

![](pilt3.png)

Kui me ei tea gruppide arvu andmestikus, siis on võimalik määrata optimaalne gruppide arv _k_. Esmalt aga mõned ettevalmistused.

## k-keskmiste klasterdamise ettevalmistused

Tunnused `kaive`, `tehinguid` ja `kesk_ost` on üsna eriskummaliste jaotustega. Neil kõigil on väga palju vaatlusi väikeste väärtustega ja "raskete sabadega" ehk mõned üksikud esktreemsed väärtused. Veendu selles eelnevalt räägitud raporti abil või jooksuta nt:
...
_Eelmiseid jooniseid on kohmakas vaadata, sest saame jooniseid teha vaid kahe tunnuse kaupa. Rakenda eelmisel õppepäeval õpitud multidimensionaalset skaleerimist._

```{r eval=FALSE, echo=FALSE}
library(MASS)
d = dist(unique(skaleeritud))
fit = isoMDS(d, k = 2)
x = fit$
y = fit$
plot(x,y, col = klastrid$cluster, main = 'Multidimensionaalselt skaleeritud jaemyyk')
```


## Klastrite eraldamine
...
kliendid$kliendi_id[klastrid$cluster == 3] # 3. klastrisse kuuluvad kliendid
kliendid$kliendi_id[klastrid$cluster == 4] # 4. klastrisse kuuluvad kliendid
```
